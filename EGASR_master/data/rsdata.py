...
class RSData(data.Dataset):
        ...
        if args.ext == 'img' or benchmark:
            ...
            if args.ext.find('reset') >= 0:
                ...
                for v in self.images_hr:
                    v = v.replace(self.ext, '.tif')
                    hr = self._read_tif(v)
                    name_sep = v.replace('.tif', '.npy')
                    np.save(name_sep, hr)
    ...
    def _read_tif(self, filename):
        ...
        img_tif = dataset.ReadAsArray(0, 0, Tif_width, Tif_height)
        img_tif = img_tif.transpose(1, 2, 0)
    ...
    def _get_patch(self, lr, hr):
        ...
        if self.train:
            ...
            # lr = common.add_noise_patch(lr, self.args.noise)
        else:
            ih, iw = lr.shape[0:2]
